...
import time
from dimicrypt import encrypt_triple_des_ecb, decrypt_triple_des_ecb, TriplePDes

if __name__ == '__main__':
    start = time.time()
    res = encrypt_triple_des_ecb('Just an encrypted message', 'akey')
    end = time.time()
    print(f'Non-parallel encryption: {(end - start):0.32f}s to generate: {res}')

    start = time.time()
    res = decrypt_triple_des_ecb(res, 'akey')
    end = time.time()
    print(f'Non-parallel decryption: {(end - start):0.32f}s to generate: {res}')

    start = time.time()
    pdes = TriplePDes('akey')
